    ...
    def totalCost(self, costs: List[int], k: int, candidates: int) -> int:
        ...
        for _ in range(k):
            ...
            if left_heap and right_heap:
                ...
                # if cost equal, check indexes
                elif left_heap[0][1] <= right_heap[0][1]:
                    cost, idx = heapq.heappop(left_heap)
                else:
                    cost, idx = heapq.heappop(right_heap)
            # if onle one heap has cand
            elif left_heap:
                cost, idx = heapq.heappop(left_heap)
            else:
                cost, idx = heapq.heappop(right_heap)
            ...
            ## add new cand 
            # we have lef and right from first pushes
            if left <= right:
                if idx < left:
                    heapq.heappush(left_heap, (costs[left], left))
                    left += 1
